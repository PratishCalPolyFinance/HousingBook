...
# (Inspired by Cédric Scherer's TidyTuesday 2019/38 script)
# ===============================================
...
# --- Load and clean Public Housing Voucher data ---
...
# --- Create 8 bins for occupancy percentages ---
df_phv_hex <- df_phv_hex %>%
  mutate(pct_bin = cut(pct_occupied,
                       breaks = c(0, 60, 70, 75, 80, 85, 90, 95, 100),
                       labels = c("0–60%", "60–70%", "70–75%", "75–80%",
...
hex_phv <- ggplot(df_phv_hex) +
  geom_sf(aes(fill = pct_bin), color = "white") +
  ...
  scale_fill_manual(values = okabe_ito,
                    ...
                    guide = guide_legend(title.position = "top",
                                         ...
                                         nrow = 1))  +
  ggtitle("A State-by-State Snapshot: How Public Housing \nVouchers Are Occupied Across the U.S.") +  # Title added with ggtitle
  theme_void() +
  ...
  guides(
    fill = guide_legend(
      ...
      keywidth = unit(1, "cm"),
      keyheight = unit(0.5, "cm"),
      nrow = 1  # Make the legend vertical by setting ncol to 1
    )
  ) +
  theme(
    plot.title = element_text(size = 25, face = "bold", hjust = 0),  # Left-align and bold title using ggtitle
  )
